...
const Header = ({
  ...
  const getUpcoming = () => {
    fetch(
      `https://api.themoviedb.org/3/movie/upcoming?api_key=60319cca340eb827a57d3235d94b1265&language=en-US&page=1`,
    )
      .then((response) => response.json())
      .then((data) => {
        setUpcomingMovies([...data.results].slice(0, 19))
      })
  }
  useEffect(() => {
    getUpcoming()
      ...
      {!disabled ? (
        ...
        >
          ...
          <section className="nav-socials">
            <a rel="noreferrer" target="_blank" href="https://myportfolio-017.netlify.app/">
              <img src={WebIcon} alt='Vercel icon' className='web-icon' />
            </a>
            <a rel="noreferrer" target="_blank" href="https://github.com/pranjalkumar2003/movies-now">
